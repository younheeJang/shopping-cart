...
const getDataLength = (datas) => {
    return datas.length
}

const cutData = (start, end, datas) => {

    const result = new Array();
    const length = getDataLength(datas)
    const descendingData = orderbyScore(datas)

    for(let i = start; i < end; i++){
        if(i === length){
            return result;
        }
        result.push(descendingData[i])
    }
    return result;
}

export const SlicedData = (currentPage, Cut) => {
    const startItemNumber = Math.ceil(currentPage*Cut) - Cut
    const endItemNumber = Math.ceil(currentPage*Cut)
    const slicedData = cutData(startItemNumber, endItemNumber, productItems)
